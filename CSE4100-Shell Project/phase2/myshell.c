...
void run_pipe(char ***pipe_res, int cnt, int pipe_num, int *previous_fd, int flag);
...
int main() 
{
    ...
    /* pipe_cmd is for storing the commands including | to use execvp. Each argument has to be null-terminated. */
...
void eval(char *cmdline, char **pipe_cmd, int is_pipe) 
{
    ...
    strcpy(buf, cmdline);
    if (is_pipe){
        char *** pipe_res = parse_pipe(&pipe_cmd, &pipe_num); //pipe_res is the parsed pipe commands which is returned by parse_pipe().
        int previous_fd = 0;
        int status_p;
        if ((pid_p = Fork()) == 0){ //child
            run_pipe(pipe_res,0,pipe_num,&previous_fd,0); //pipe_num is assigned in parse_pipe().  
            exit(0);
        }
        //parent
        ...
        if (!builtin_command(argv)) { //quit -> exit(0), & -> ignore, other -> run, if command has pipe in it -> fork an another process for piping
...
char*** parse_pipe(char ***pipe_cmd, int *pipe_num)
{
    int i, j;
    char *argv[MAXARGS];
    char sliced[MAXARGS][MAXLINE]; //Sliced strings by '|'
    ...
    }//allocate the space needed for pipe_args
    ...
    } //Slice the strings by '|' and store it in sliced[] array
    for(i=0; i<cnt-1; i++)
...
{  
    if (flag == 1){
        exit(0); //terminate all piped process as SIGINT was received.
    }
    int i;
    ...
    line[i] = NULL; //null-terminate the last argument of the line.  
    strcat(program_path, line[0]); // ex) /bin/ls
    int pid, status;
    int pfd[2];
    if (cnt == pipe_num) 
        exit(0); //If there are no pipe left, terminate.
    pipe(pfd);
    if ((pid = Fork()) == 0){
        dup2(*previous_fd, 0); //copy file descriptor value from the previous pipe command output received by the parent process.
        if (cnt+1 != pipe_num) //When there are no more pipe, we don't need to give our command's output to the next pipe.
            dup2(pfd[1], 1);
        close(pfd[0]); //close pfd[0] which is not used.
    ...
    close(pfd[1]);
    *previous_fd = pfd[0]; //save pfd which received the output of the previous command.
    run_pipe(pipe_res,cnt+1,pipe_num,previous_fd, flag); //recursively repeat until there are no more pipe left.
}
